...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Add the 'creation_time' column to the INSERT statement and set it to the current timestamp
    $sql = "INSERT INTO Users (name, email, password, security_question, security_answer, creation_time) VALUES (?, ?, ?, ?, ?, NOW())";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$name, $email, $password, $security_question, $security_answer]);
...
<head>
  ...
  <title>NBA Player Search - Register</title>
...
<body>
  <form method="POST" action="register.php">
    <h1 style="text-align: center;">NBA Player Search - Register</h1>
